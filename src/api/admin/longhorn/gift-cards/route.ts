...
import type { LonghornCreateGiftCardRequest } from "../../../../types/longhorn-request-types";
...
export const POST = async (
  req: LonghornCreateGiftCardRequest,
  res: MedusaResponse
) => {
  try {
    ...
    const giftCardId = `gc_${Date.now()}_${Math.random()
      .toString(36)
      .substring(2, 8)}`;
    ...
    const transactionId = `gct_${Date.now()}_${Math.random()
      .toString(36)
      .substring(2, 8)}`;
