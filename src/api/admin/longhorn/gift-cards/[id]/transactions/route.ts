...
import type {
  ...
  LonghornCreateGiftCardTransactionRequest,
...
export const POST = async (
  req: LonghornCreateGiftCardTransactionRequest,
  res: MedusaResponse
) => {
  try {
    ...
    if (
      !(Object.values(GIFT_CARD_TRANSACTION_TYPES) as string[]).includes(
        transaction_type
      )
    ) {
      return res.status(400).json({
    ...
    const [createdTransaction] =
      await longhornService.createLonghornGiftCardTransactions([
